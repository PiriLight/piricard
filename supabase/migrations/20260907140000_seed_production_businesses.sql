-- =============================================================================
-- PiriCard — Phase 4D: production seed for the four real, currently-published
-- PiriCards.
--
-- Migrates autoformigal, beauty-connection-360, boi-na-brasa, oft-racing from
-- lib/businesses.ts into Supabase. Deliberately does NOT create `pirilight`
-- (an incomplete internal profile, not a production PiriCard).
--
-- This is the versioned, applied-through-the-normal-migration-path copy of
-- the identical, already locally-validated supabase/seed.sql (Phase 4C —
-- see that file's own header for full data-provenance notes). `seed.sql`
-- itself is NOT applied to remote projects by `supabase db push`/migration
-- tooling (it's a local-dev-only `db reset` convenience) — this file exists
-- so the exact same INSERTs go through the normal, audited, reviewable
-- migration path instead of an untracked ad-hoc script.
--
-- Runs as the migration-apply role (table owner), so it bypasses RLS by
-- construction — the same way the schema migration itself does. This is
-- "trusted SQL", the intended write path for `businesses` content (the
-- table has zero INSERT grant to any application role — see
-- 20260906210142_piricard_v1_schema.sql section 6.1). Does NOT go through
-- the admin RPCs (admin_create_business etc.): those require an
-- authenticated platform-admin session, which a migration has no need for.
--
-- IDEMPOTENCY: each business is wrapped in `if not exists (... where slug =
-- ...)` — re-running this migration (or a later `supabase db reset` that
-- also runs seed.sql) skips every business already present instead of
-- erroring or duplicating rows.
--
-- NO schema change, NO RLS change, NO auth-user change, NO Storage change,
-- NO business_memberships rows. Content is byte-identical to
-- supabase/seed.sql's four `do $$ ... $$` blocks (autoformigal,
-- beauty-connection-360, boi-na-brasa, oft-racing) — verified against
-- lib/businesses.ts and field-by-field parity-checked in Phase 4C
-- (PARITY_REPORT.md).
-- =============================================================================
