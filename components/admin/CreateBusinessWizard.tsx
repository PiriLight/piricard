...
import { buildCreatePreviewBusiness, MODULE_KEYS } from "@/lib/admin/business-mapping";
...
import PreviewPane from "@/components/admin/business-editor/PreviewPane";
import ImageUploadField from "@/components/admin/business-editor/ImageUploadField";
...
export default function CreateBusinessWizard() {
  ...
  // Phase 4H: a stable per-session key so logo/cover can be uploaded to
  // Supabase Storage BEFORE the business row exists — scoped under
  // businesses/draft-{draftKey}/... (never the bucket root, never another
  // business's folder). Lazily generated once; never recreated on re-render.
  ...
  // Phase 4G — same canonical mapper the edit-mode preview uses
  // (buildCreatePreviewBusiness -> mapBusinessFromDatabase/mapModuleActivation
  // /buildPreviewBusiness), fed with the wizard's current in-memory values.
  // No Supabase read, no database id — recomputed on every keystroke/toggle.
  const previewBusiness = useMemo(
    () =>
      buildCreatePreviewBusiness({
        name,
        category,
        slug,
        layoutVariant,
    ...
    [name, category, slug, layoutVariant, primaryColor, accentColor, logo, cover, enabledModules],
  );
        ...
        <div className={`admin-editor-form-pane${mobileView === "edit" ? " is-active-mobile" : ""}`}>
          <form className="admin-create-wizard" onSubmit={handleSubmit}>
            ...
            <fieldset className="admin-fieldset">
              ...
              <div className="admin-field">
                <label htmlFor="new-logo">Logótipo</label>
                <ImageUploadField businessKey={`draft-${draftKey}`} kind="logo" label="logótipo" currentValue={logo} onUploaded={(publicUrl) => setLogo(publicUrl)} />
                <input id="new-logo" type="text" placeholder="/clients/exemplo/logo.png" value={logo} onChange={(event) => setLogo(event.target.value)} />
              </div>
              <div className="admin-field">
                <label htmlFor="new-cover">Imagem de capa</label>
                <ImageUploadField businessKey={`draft-${draftKey}`} kind="cover" label="capa" currentValue={cover} onUploaded={(publicUrl) => setCover(publicUrl)} />
